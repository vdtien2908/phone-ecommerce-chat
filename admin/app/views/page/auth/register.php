<section class="min-vh-100 mb-8">
    <div class="page-header align-items-start min-vh-50 pt-5 pb-11 mx-3 border-radius-lg" style="background-image: url('<?php echo SCRIPT_ROOT . '/assets/img/curved-images/curved14.jpg' ?>');">
        <span class="mask bg-gradient-dark opacity-6"></span>
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-lg-5 text-center mx-auto">
                    <h1 class="text-white mb-2 mt-5">Chào mừng!</h1>
                    <p class="text-lead text-white">Tạo tài khoản của bạn chỉ với vài bước đơn giản.</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="row mt-lg-n10 mt-md-n11 mt-n10">
            <div class="col-xl-4 col-lg-5 col-md-7 mx-auto">
                <div class="card z-index-0">
                    ...
                    <div class="card-body">
                        <form role="form text-left" method="POST" action="/phone-ecommerce-chat/admin/auth/signUp">
                            <div class="mb-3">
                                <input type="text" class="form-control" placeholder="Họ và tên" name="customer_name" id="customer_name" aria-label="customer_name" aria-describedby="customer_name" value="<?php echo isset($_SESSION['old']['customer_name']) ? htmlspecialchars($_SESSION['old']['customer_name']) : ''; ?>">
                                <?php if (isset($_SESSION['errors']['customer_name'])) : ?>
                                    <p class="text-danger text-xs mt-2"><?php echo $_SESSION['errors']['customer_name']; ?></p>
                                    <?php unset($_SESSION['errors']['customer_name']);
                                    ?>
                                <?php endif; ?>
                            </div>
                            <div class="mb-3">
                                <input type="email" class="form-control" placeholder="Email" name="email" id="email" aria-label="Email" aria-describedby="email-addon" value="<?php echo isset($_SESSION['old']['email']) ? htmlspecialchars($_SESSION['old']['email']) : ''; ?>">
                                <?php if (isset($_SESSION['errors']['emailErr'])) : ?>
                                    <p class="text-danger text-xs mt-2"><?php echo $_SESSION['errors']['emailErr']; ?></p>
                                    <?php unset($_SESSION['errors']['emailErr']); ?>
                                <?php endif; ?>
                            </div>
                            <div class="mb-3">
                                ...
                                <?php if (isset($_SESSION['error']['passwordEr'])) : ?>
                                    <p class="text-danger text-xs mt-2"><?php echo $_SESSION['error']['passwordEr']; ?></p>
                                    <?php unset($_SESSION['error']['passwordEr']); ?>
                                <?php endif; ?>
                            </div>
                            <div class="form-check form-check-info text-left">
                                <input class="form-check-input" type="checkbox" name="agreement" id="flexCheckDefault" checked>
                                <label class="form-check-label" for="flexCheckDefault">
                                    Đồng ý với <a href="javascript:;" class="text-dark font-weight-bolder">các điều khoản</a>
                                </label>
                                <?php if (isset($_SESSION['errors']['agreement'])) : ?>
                                    <p class="text-danger text-xs mt-2">Trước hết, đồng ý với các điều khoản, sau đó hãy thử lại.</p>
                                    <?php unset($_SESSION['errors']['agreement']); ?>
                                <?php endif; ?>
                            </div>
                            <div class="text-center">
                                <button type="submit" class="btn bg-gradient-dark w-100 my-4 mb-2">Đăng ký</button>
                            </div>
                            <p class="text-sm mt-3 mb-0">Đã có tài khoản? <a href="<?php echo URL_APP . '/auth/login' ?>" class="text-dark font-weight-bolder">Đăng nhập</a></p>
                        </form>
                    </div>
                </div>
            </div>
